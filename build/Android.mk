...
# 3. Update PYTHONPATH to include external folder, so autotest can import chromite
#    module from external/chromite folder.
# 4. Run control_file_preprocessor.py to create a file mapping suites to the
#    test control files for each suite.
# 5. tar the autotest intermediate folder as the desired test_suites package.
$(test_suites_package) : $(AUTOTEST_TEST_SUITES)
	...
	$(hide) export PYTHONPATH=${PYTHONPATH}:external && \
		python -B $(AUTOTEST_DIR)/site_utils/control_file_preprocessor.py \
